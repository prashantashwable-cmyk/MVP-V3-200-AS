/**
 * API routes. Every route resolves the caller from a session token and is
 ...
 *
 * Framework-agnostic: mounted on Express by api.ts (Node server) and on a
 * tiny in-page router by web/main.ts (browser demo build).
 */
...
export interface Req { headers: Record<string, any>; params: Record<string, string>; body: any }
...
export type Handler = (req: any, res: any, next: NextFunction) => unknown;
export interface MiniRouter {
  get(path: string, ...handlers: Handler[]): unknown;
  post(path: string, ...handlers: Handler[]): unknown;
  use(handler: Handler): unknown;
}
type Request = Req;
type Response = Res;
type Authed = Req & { user: UserRow };
...
export function registerRoutes(r: MiniRouter, engine: Engine, director?: DemoDirector): void {
  ...
  r.get('/evidence/:id', auth, h((req, res) => {
    ...
    res.end(m[2] ? Uint8Array.from(atob(m[3]), c => c.charCodeAt(0)) : decodeURIComponent(m[3]));
  }));
  ...
  r.use((_req: Req, res: Res) => { res.status(404).json({ error: 'NOT_FOUND', message: 'No such endpoint' }); });
}
